...
export default function AgendaHoy() {
  const { data, error, isLoading } = useSWR("/api/dashboard/hoy", fetcher, {
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Circuito</th>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((caminata: any) => (
            <tr key={caminata.id}>
              ...
              <td className="px-6 py-4">{Math.round(caminata.ocupacion * 100)}%</td>
