...
type ApiError = { error: { message: string; code?: string; details?: any } };
...
export function forbidden(message = "Forbidden") {
  const body: ApiError = { error: { message, code: "FORBIDDEN" } };
  return NextResponse.json(body, { status: 403 });
}

export function notFound(message = "Not found") {
  const body: ApiError = { error: { message, code: "NOT_FOUND" } };
...
export function errorResponse(e: unknown) {
  ...
  if (e && typeof e === "object" && (e as any).message) {
    ...
    const body: ApiError = { error: { message: (e as any).message || "Error", code: maybeCode } };
  ...
  const body: ApiError = { error: { message: "Internal server error", code: "INTERNAL_SERVER_ERROR" } };
